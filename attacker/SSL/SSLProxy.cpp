...
int SSLProxy::SSL_ProxyBoth(LPSSLPROXYPARAM pstSSLProxyParam) {
	int				iCounter = 0;
	int				iRet = 0;
	...
	if (dwip == 0) {
		...
		Public::writeFile(ATTACK_LOG_FILENAME, (unsigned char *)recvBuffer, iCounter, szout);
		printf(szout);
#endif
		return FALSE;
	}
	...
	if (iRet != 1)
	{
		printf("SSLProxy %s SSL_set_fd errorcode:%d,error string:%s,description:%s,return:%d\n",pstSSLProxyParam->host,
			SSL_get_error(pstSSLProxyParam->SSLToServer, iRet), SSL_state_string(pstSSLProxyParam->SSLToServer),
...
int SSLProxy::SSL_ProxyClient(LPSSLPROXYPARAM pstSSLProxyParam) {
	...
	if (iRet != 1)
	{
		printf("SSL_set_fd %s errorcode:%d,error string:%s,description:%s,return:%d\n", pstSSLProxyParam->host,
			SSL_get_error(pstSSLProxyParam->SSLToClient, iRet), SSL_state_string(pstSSLProxyParam->SSLToClient),
	...
	iRet = SSL_ProxyBoth(pstSSLProxyParam);
...
int __stdcall SSLProxy::SSL_Proxy(MIM_THREAD_PARAMS * param) {
	...
	while (TRUE)
	{
		__try
		{
			...
			ret = SSL_ProxyClient(pstSSLProxyParam);
