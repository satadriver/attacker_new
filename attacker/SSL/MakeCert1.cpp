...
int MakeCert::makeSuperCACRT(string keyfn,string password,
	string c,string st,string l,string o,string ou,string cn,string e,string cfgfn,string outcrtfn) {
	char * cmdformat =
...
int MakeCert::makeExtCRT(string cfgpath,string csrfn, string password, string cacertfn, string certfn, string cakeyfn) {
	...
	char * signcmdformat = "openssl ca -extensions SAN -config %s -passin pass:%s -in %s -out %s -cert %s -keyfile %s -days 3650 -policy policy_anything -batch";
	char szcmd[1024];
